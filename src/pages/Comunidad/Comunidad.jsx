import { Box, Stack } from '@chakra-ui/react';
import CustomSlider from '../../components/CustomSlider/CustomSlider';
import CardSlider from './components/CardSlider';
import HeaderComunidad from './components/HeaderComunidad';

const dataS = [
	{
		title: 'Antes de salir de viaje',
		number: '1',
		blog: [
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/voluntariado-por-el-mundo.jpg',
				imgAlt: 'voluntariado-por-el-mundo',
				headTitle: 'tips de viaje',
				title: '5 formas de hacer voluntariados y viajar por el mundo',
				id: 1,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2013/02/trabajar-como-camarero.jpg',
				imgAlt: 'camarero-por-el-mundo',
				headTitle: 'tips de viaje',
				title: 'Consejos para trabajar de camarero por el mundo',
				id: 2,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2020/06/que-llevar-en-valija-de-mano.jpg',
				imgAlt: 'que-llevar-en-valija-de-mano',
				headTitle: 'tips de viaje',
				title: 'Cosas útiles: Qué llevar en la valija de mano?',
				id: 3,
			},
		],
	},
	{
		title: 'Acerca de tu destino',
		number: '2',
		blog: [
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2013/03/conocer-clima-ciudad-viaje.jpg',
				imgAlt: 'conocer-clima-ciudad-viaje',
				headTitle: 'tips de viaje',
				title: 'Conocer el clima de una ciudad antes y durante un viaje!',
				id: 2,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/en-que-pais-se-puede-ahorrar-mas-2.jpg',
				imgAlt: 'en-que-pais-se-puede-ahorrar-mas',
				headTitle: 'tips de viaje',
				title:
					'¿En qué país se puede ahorrar más con una visa Working Holiday?',
				id: 1,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/mejores-oficios-para-trabajar-por-el-mundo.jpg',
				imgAlt: 'mejores-oficios-para-trabajar-por-el-mundo',
				headTitle: 'tips de viaje',
				title: 'Los mejores oficio para viajar y trabajar por el mundo',
				id: 3,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2015/12/cepo-dolares-extraer-dinero-pagar-tarjeta-exterior.jpg',
				imgAlt: 'cepo-dolares-extraer-dinero-pagar-tarjeta-exterior',
				headTitle: 'tips de viaje',
				title:
					'Cómo extraer y comprar dólares, pagar con tarjeta en el exterior [CEPO 2022]',
				id: 4,
			},
		],
	},
	{
		title: 'Documentos importantes',
		number: '3',
		blog: [
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2013/05/como-sacar-la-licencia-de-conducir-internacional.jpg',
				imgAlt: 'como-sacar-la-licencia-de-conducir-internacional',
				headTitle: 'tips de viaje',
				title: '¿Cómo sacar la licencia de conducir internacional?',
				id: 2,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2013/06/pasaporte-doble-ciudadania.jpg',
				imgAlt: 'pasaporte-doble-ciudadania',
				headTitle: 'tips de viaje',
				title: '¿Qué pasaporte usar si voy a viajar y tendo doble cuidadania?',
				id: 1,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2014/10/como-certificar-un-titulo.jpg',
				imgAlt: 'como-certificar-un-titulo',
				headTitle: 'tips de viaje',
				title: '¿Cómo certificar un título para trabajar en el exterior?',
				id: 3,
			},
		],
	},
	{
		title: 'Más tips para tu viaje',
		number: '4',
		blog: [
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2013/10/ingles-ielts-toefl-rendir-examen-ingles.jpg',
				imgAlt: 'ingles-ielts-toefl-rendir-examen-ingles',
				headTitle: 'tips de viaje',
				title:
					'IELTS, TOEFL, FIRST: ¿Qué examen internacional de inglés me conviene rendir?',
				id: 2,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2016/12/09-que-tener-en-cuenta-para-viajar-en-una-van-2.jpg',
				imgAlt: 'que-tener-en-cuenta-para-viajar-en-una-van',
				headTitle: 'tips de viaje',
				title:
					'10 cosas que deberías chequear antes de salir de viaje en auto/van',
				id: 1,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/mejores-aplicaciones-para-viajar-por-el-mundo.jpg',
				imgAlt: 'mejores-aplicaciones-para-viajar-por-el-mundo',
				headTitle: 'tips de viaje',
				title: 'Las mejores aplicaciones para viajar por el mundo!',
				id: 3,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/antecedentes-penales-como-obtenerlos-argentina.jpg',
				imgAlt: 'antecedentes-penales-como-obtenerlos-argentina',
				headTitle: 'tips de viaje',
				title: 'Cómo obetener el certificado de antecedentes penales (ARG)',
				id: 4,
			},
			{
				imgUrl:
					'https://www.yomeanimo.com/wp-content/uploads/2021/11/como-sacar-buenas-fotos-2.jpg',
				imgAlt: 'como-sacar-buenas-fotos',
				headTitle: 'tips de viaje',
				title: '¿Cómo sacar  buenas fotos de tu viaje con el celular?',
				id: 5,
			},
		],
	},
];

const Comunidad = () => {
	return (
		<Box bg='#f1f1f1' minH='100vh'>
			<HeaderComunidad />
			<Stack py='20' px='10' gap='14'>
				{dataS?.map(element => (
					<CustomSlider
						key={element.number}
						number={element.number}
						title={element.title}
					>
						{element?.blog?.map(element => (
							<CardSlider
								key={element.id}
								imgUrl={element.imgUrl}
								imgAlt={element.imgAlt}
								headTitle={element.headTitle}
								title={element.title}
							/>
						))}
					</CustomSlider>
				))}
			</Stack>
		</Box>
	);
};

export default Comunidad;
